...
public class HelperClass {
    ...
    public byte[] decrypt(byte[] encryptedBytes)
        ...
        return cipher(encryptedBytes, skey, this.ivBytes, false);
    }

    public byte[] decrypt(byte[] encryptedBytes, byte[] encryptionKey, byte[] ivBytes)
            throws InvalidCipherTextException {
        return cipher(encryptedBytes, encryptionKey, ivBytes, false);
    }
    ...
    private byte[] cipher(byte[] sourceContent, byte[] secureKey, byte[] ivBytes, boolean encryptOperation) throws InvalidCipherTextException {
        ...
        int outputSize;
        if(encryptOperation){
            outputSize = gcmb.getOutputSize(sourceContent.length);
        }else {
            outputSize = gcmb.getOutputSize(sourceContent.length);
        }
    ...
    public String getSignedXml(Auth auth) throws Exception {
...
//                ignoreExtraElements);
    ...
    public void saveIvBytes(String timestamp) {
        this.ivBytes = timestamp.getBytes();
    }

    public void saveSkey(byte[] skey){
        this.skey = skey;
    }
}
